...
class GcmPgEncryption {
    ...
    async encryptWithMKeys(plainMessage) {
        ...
        // IMPORTANT:
        // The bank-provided React PG Kit returns Base64 here.
...
export const encryptGetePay = async (plainText, { iv, key }) => {
    const gcm = new GcmPgEncryption(iv, key);
    return gcm.encryptWithMKeys(plainText);
};

export const decryptGetePay = async (cipherText, { iv, key }) => {
    if (!cipherText) {
        throw new Error("GetePay encrypted response is empty.");
    }

    const gcm = new GcmPgEncryption(iv, key);
    return gcm.decryptWithMKeys(String(cipherText).trim());
};

export const parseGetePayResponse = async (encryptedResponse, credentials) => {
    const plaintext = await decryptGetePay(encryptedResponse, credentials);

    try {
        return JSON.parse(plaintext);
    } catch {
        throw new Error("GetePay response decrypted but is not valid JSON.");
    }
};
